/* s1_prof.c - instrumented copy of the S1 machine for P6 diagnostics. This is
 * NOT the frozen substrate (s1.c); it exists only so the profiling build can
 * count dynamic opcode/HOST events without modifying the frozen files. The
 * semantics are identical to s1.c. */
#include "s1.h"
#include "s1_prof.h"
