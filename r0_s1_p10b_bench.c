/* r0_s1_p10b_bench.c - FIB-OPT-P10B/P10C/P10D: compiled-S1 variants.
 *
...
static int build_so(const char *cpath, const char *sopath, cell code_end, int mode) {
    /* mode: 0 = P10A, 1 = P10B, 2 = P10C, 3 = P10D */
...
int main(void) {
    ...
    /* correctness: P10C and P10D must equal interpreted on all workloads. */
    {
        cell blocks[8];
        const char *names[8];
        ...
        for (int i = 0; i < 8; i++) {
            ...
            int match = (Ni == Nc && Ri == Rc && Ni == Nd && Ri == Rd);
            printf("  %s: interp N=%d  P10C N=%d r=%ld  P10D N=%d r=%ld  %s\n",
                   names[i], Ni, Nc, (long)Rc, Nd, (long)Rd, match ? "MATCH" : "MISMATCH");
            if (!match) ok = 0;
        }
    ...
    static double tc[8], td[8];
    int got = 0;
    for (int r = 0; r < 8; r++) {
        double t0 = now_sec(); r0_s1_run_compiled(fib_block, run_p10c); double t1 = now_sec();
        tc[got] = t1 - t0;
        t0 = now_sec(); r0_s1_run_compiled(fib_block, run_p10d); t1 = now_sec();
        td[got] = t1 - t0;
        got++;
    }
    for (int i = 0; i < got; i++)
