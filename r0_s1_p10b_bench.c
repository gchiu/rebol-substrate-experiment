/* r0_s1_p10b_bench.c - FIB-OPT-P10B..P10F: compiled-S1 variants.
 *
 ...
 *   - P10F: P10E plus sequential fall-through (dispatch elimination).
 *
...
/* ============================ P10F emitter ===============================
 * Sequential fall-through / dispatch elimination.
 *
 * Same as P10E, except sequential S1 instructions no longer end with
 * `ip = next; goto *T[ip-256]` -- they fall through in generated C. Only
 * genuine control flow (0BRANCH, the LIT 0 ! branch idiom, HALT, error) keeps
 * an explicit dispatch; HALT/DUMP set `ip` to the current address first so the
 * recorded IP stays correct. */

static void emit_compiled_c_p10f(FILE *f, cell code_end) {
    cell n = code_end - CODE_BASE;
    unsigned char *instr = (unsigned char *)calloc((size_t)(n > 0 ? n : 1), 1);
    for (cell a = CODE_BASE; a < code_end; ) {
        instr[a - CODE_BASE] = 1;
        ...
        case OP_LIT: {
            ...
            if (operand >= 0 && operand <= 2 &&
                ...
                if (next_op == OP_FETCH) {
                    ...
                    /* write IP (branch): explicit dispatch */
                    fputs("    ip = tos; tos = nos; nos = M[sp++];\n    goto *T[ip - 256];\n\n", f);
                    a += 3;
                } else if (operand == 1) {
                    fputs("    { cell _v = tos; tos = M[_v]; nos = M[_v + 1]; sp = _v + 2; }\n", f);
...
static int build_so(const char *cpath, const char *sopath, cell code_end, int mode) {
    /* mode: 0 = P10A, 1 = P10B, 2 = P10C, 3 = P10D, 4 = P10E, 5 = P10F */
    ...
    else if (mode == 5) emit_compiled_c_p10f(cf, code_end);
...
int main(void) {
    ...
    if (build_so("/tmp/opencode_p10f.c", "/tmp/opencode_p10f.so", code_end, 5) != 0) {
        fprintf(stderr, "P10F build failed\n"); return 1;
    }
    double c1 = now_sec();
    ...
    void (*run_p10f)(cell *, cell) = (void (*)(cell *, cell))load_so("/tmp/opencode_p10f.so");

    /* correctness: P10E and P10F must equal interpreted on all workloads. */
    {
        cell blocks[8];
        const char *names[8];
        ...
        for (int i = 0; i < 8; i++) {
            ...
            int Nf = r0_s1_run_compiled(blocks[i], run_p10f);
            cell Rf = (Nf == 1) ? r0_s1_result(0, Nf) : -999;
            int match = (Ni == Ne && Ri == Re && Ni == Nf && Ri == Rf);
            printf("  %s: interp N=%d  P10E N=%d r=%ld  P10F N=%d r=%ld  %s\n",
                   names[i], Ni, Ne, (long)Re, Nf, (long)Rf, match ? "MATCH" : "MISMATCH");
            if (!match) ok = 0;
        }
    ...
    r0_s1_run_compiled(fib_block, run_p10f);

    static double te[8], tf[8];
    int got = 0;
    for (int r = 0; r < 8; r++) {
        double t0 = now_sec(); r0_s1_run_compiled(fib_block, run_p10e); double t1 = now_sec();
        te[got] = t1 - t0;
        t0 = now_sec(); r0_s1_run_compiled(fib_block, run_p10f); t1 = now_sec();
        tf[got] = t1 - t0;
        got++;
    }
    for (int i = 0; i < got; i++)
        for (int j = i + 1; j < got; j++) {
            ...
            if (tf[j] < tf[i]) { double t = tf[i]; tf[i] = tf[j]; tf[j] = t; }
        }
    double me = te[got/2], mf = tf[got/2];
    printf("fib 25 P10E (TOS+NOS):     median %.4f s (min %.4f, max %.4f)\n", me, te[0], te[got-1]);
    printf("fib 25 P10F (fallthrough): median %.4f s (min %.4f, max %.4f)\n", mf, tf[0], tf[got-1]);
    printf("speedup (P10E/P10F): %.3fx\n", me / mf);
